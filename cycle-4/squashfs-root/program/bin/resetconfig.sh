...
ResetConfig()
{
	...
	# reserve admin passwd config
	#WebLoginPasswd,WebLoginPasswd20 含有特殊字符,统一对非数字和字母符号添加转义
	WebLoginPasswd=`sed -n 's/<WebLoginPasswd>\(.*\)<\/WebLoginPasswd>/\1/p' $CONFA|sed s/[[:space:]]//g|sed 's/[^0-9a-fA-f]/\\\&/g'`
	WebLoginPasswd20=`sed -n 's/<WebLoginPasswd20>\(.*\)<\/WebLoginPasswd20>/\1/p' $CONFA|sed s/[[:space:]]//g|sed 's/[^0-9a-fA-f]/\\\&/g'`

	echo reserve WebLoginPasswd WebLoginPasswd20
	...
	DdeviceSL=`grep \<Device\> $CONFA -n | awk -F ':' '{print $1}'`                                                          
	DdeviceEL=`grep \<\/Device\> $CONFA -n | awk -F ':' '{print $1}'` 
	if [ -z "$WebLoginPasswd" ]; then
		echo there is empty WebLoginPasswd param!
	else
		sed -i $DdeviceSL,$DdeviceEL's/<WebLoginPasswd>.*</<WebLoginPasswd>'"$WebLoginPasswd"'</g' $CONFA
	fi
	
	if [ -z "$WebLoginPasswd20" ]; then
		echo there is empty WebLoginPasswd20 param!
	else
		sed -i $DdeviceSL,$DdeviceEL's/<WebLoginPasswd20>.*</<WebLoginPasswd20>'"$WebLoginPasswd20"'</g' $CONFA
	fi
